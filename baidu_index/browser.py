...
class BaiduBrowser(object):
    ...
    def _get_area_cd(self,area_name):
        if not area_name:
            area_cd = 0 
        else:
            cities = config.get_cities()
            s_cds = [city[0] for city in cities if city[1] == area_name]
            area_cd = s_cds[0] if len(s_cds) > 0 else 0
        return area_cd
    ...
    def get_one_day_index(self, date,keyword,city,type_name, url):
        """
           从图片自动识别数字
        """
        try_num = 0
    ...
    def get_baidu_index_by_date_range(self, keyword, city, start_date, end_date,bdi_types):
        # 根据区间获取关键词的索引值
        url = config.time_range_trend_url.format(
            start_date=start_date, end_date=end_date,
            word=urllib.parse.quote(keyword.encode('gbk')),
            area =  self._get_area_cd(city)
            )
        ...
        # 获取指定区间的日期列表,方便下面循环用
        start_date, end_date, date_list = self.get_date_info(start_date, end_date)

        # 拼接api的url
        url = config.all_index_url.format(res=res, res2=res2, start_date=start_date, end_date=end_date)
        # 获取api的结果信息，这里面保存了后面日期节点的一些加密值
        all_index_info = self.api.get_all_index_html(url)
        wm = WorkManager(config.num_of_threads)

        for type_name in bdi_types:
            for item in all_index_info['data'][type_name]:
                if city != item['areaName']:
                    continue

                indexes_enc = item['userIndexes_enc']
                enc_list = indexes_enc.split(',')

                print("开始第日数据下载..%s,%s,%s"% (keyword,city,type_name))

                # 遍历这些enc值，这些值拼接出api的url(这个页面返回 图片信息以及css规定的切图信息)
                for index, _ in enumerate(enc_list):
                    url = config.index_show_url.format(res=res, res2=res2, enc_index=_, t=int(time.time()) * 1000)
                    # 根据enc在列表中的位置，获取它的日期
                    date = date_list[index]
                    # 将任务添加到多线程下载模型中
                    wm.add_job(date, self.get_one_day_index, date,keyword,city,type_name, url)

        wm.start()
        wm.wait_for_complete()

        # 执行结束后，从结果queue中获取到最终的百度指数字典
        baidu_index_list = wm.get_all_result_from_queue()

        return baidu_index_list

    

    def _get_index_period(self, keyword,city):
        # 拼接一周趋势的url
        url = config.one_week_trend_url.format(
            word = urllib.parse.quote(keyword.encode('gbk')),
            area = urllib.parse.quote(self._get_area_cd(city))
            )
        self.browser.get(url)
        # 获取下方api要用到的res和res2的值
        res = self.browser.execute_script('return PPval.ppt;')
        res2 = self.browser.execute_script('return PPval.res2;')
        start_date, end_date = self.browser.execute_script('return BID.getParams.time()[0];').split('|')
        start_date, end_date, date_list = self.get_date_info(start_date, end_date)
        url = config.all_index_url.format(res=res, res2=res2, start_date=start_date, end_date=end_date)
        all_index_info = self.api.get_all_index_html(url)
    ...
    def get_baidu_index(self, keyword, city, bdi_types):
        if config.start_date and config.end_date:
            _, _, date_list = self.get_date_info(start_date=config.start_date, end_date=config.end_date)
        else:
            # 配置文件不配置start_date和end_date，可以查询到这个关键词数据的最大区间
            date_list = self._get_index_period(keyword,city)
        ...
        while start < end:
            try:
                ...
                result = self.get_baidu_index_by_date_range(keyword,city, start_date, end_date, bdi_types)
                baidu_index_list.extend(result)
                start += skip + 1
            except:
                import traceback
    ...
    def login(self, user_name, password):
        ...
        user_name_obj = self.browser.find_element_by_id('TANGRAM__PSP_3__userName')
